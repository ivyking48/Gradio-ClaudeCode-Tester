"""BUG VARIANT B: 'Set Start/End from video' buttons use DOM manipulation
instead of the js= pattern. They always set 0 because Gradio's Svelte
state overrides the DOM change."""
...
""") as demo:
    ...
    # BUG: These buttons use DOM manipulation instead of js= parameter.
    # The Svelte state overrides the DOM change, so trim always stays at default.
    btn_set_start.click(fn=lambda ts: ts, inputs=trim_start_inp, outputs=trim_start_inp)
    btn_set_end.click(fn=lambda ts: ts, inputs=trim_end_inp, outputs=trim_end_inp)

    # Load JS that tries (and fails) to sync via DOM
    demo.load(js="""() => {
        document.querySelector('#trim-start')?.closest('.gradio-component')
            ?.querySelector('button')?.addEventListener('click', () => {
            const v = document.getElementById('test-video');
            const inp = document.querySelector('#trim-start input');
            if (v && inp) { inp.value = v.currentTime.toFixed(1); inp.dispatchEvent(new Event('input', {bubbles:true})); }
        });
        document.querySelector('#trim-end')?.closest('.gradio-component')
            ?.querySelector('button')?.addEventListener('click', () => {
            const v = document.getElementById('test-video');
            const inp = document.querySelector('#trim-end input');
            if (v && inp) { inp.value = v.currentTime.toFixed(1); inp.dispatchEvent(new Event('input', {bubbles:true})); }
        });
    }""")
